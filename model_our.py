...
class Model(nn.Module):
    ...
    def forward(self, U, qmask, umask, seq_lengths, Sentence, clip_model, U_a=None, U_v=None, epoch=None):
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            neg_emotions_feat = self.dropout_(neg_emotions_feat)
            neg_emotions_feat = nn.ReLU()(neg_emotions_feat)
            neg_log_prob = F.log_softmax(self.smax_fc(neg_emotions_feat), 1)

            neg_emotions_feat2 = self.dropout_(neg_emotions_feat2)
            neg_emotions_feat2 = nn.ReLU()(neg_emotions_feat2)
            neg_log_prob2 = F.log_softmax(self.smax_fc(neg_emotions_feat2), 1)

            neg_emotions_feat3 = self.dropout_(neg_emotions_feat3)
            neg_emotions_feat3 = nn.ReLU()(neg_emotions_feat3)
            neg_log_prob3 = F.log_softmax(self.smax_fc(neg_emotions_feat3), 1)
