...
# We use seed = 1475 on IEMOCAP and seed = 67137 on MELD
seed = 1475
...
def train_or_eval_graph_model(model, clip_model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
    ...
    return avg_loss, avg_accuracy, labels, preds, avg_fscore, avg_micro_fscore, avg_macro_fscore, vids, ei, et, en, el
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_L', type=int, default=5, help='num_hyperconvs') # 3

    parser.add_argument('--num_K', type=int, default=4, help='num_convs') # 4
    ...
    for e in range(n_epochs):
        ...
        if args.graph_model:
            train_loss, train_acc, _, _, train_fscore, train_avg_micro_fscore, train_avg_macro_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, clip_model, loss_function, train_loader, e, cuda, args.modals, optimizer, True, dataset=args.Dataset)
            valid_loss, valid_acc, _, _, valid_fscore, valid_avg_micro_fscore, valid_avg_macro_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, clip_model, loss_function, valid_loader, e, cuda, args.modals, dataset=args.Dataset)
            test_loss, test_acc, test_label, test_pred, test_fscore, test_avg_micro_fscore, test_avg_macro_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, clip_model, loss_function, test_loader, e, cuda, args.modals, dataset=args.Dataset)
